...
extension SIMChatViewController: SIMChatInputBarDelegate, SIMChatInputPanelToolBoxDelegate, SIMChatInputPanelEmoticonViewDelegate, SIMChatInputPanelAudioViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, SIMChatMapViewControllerDelegate {
    ...
    public func inputPanel(inputPanel: UIView, didSelectToolBoxItem item: SIMChatInputItemProtocol) {
        ...
        if(item.itemIdentifier == "page:location") {
            let mapVC = SIMChatMapViewController()
            mapVC.delegate = self
    ...
    //MARK: SIMChatMapViewControllerDelegate 
    
    public func shareButtonPressed(locationShared: CLLocation, locationNameShared: String) {
        let content = SIMChatBaseMessageMapContent(location: locationShared, size: CGSizeMake(150,150), locationName: locationNameShared)
        self.messageManager.sendMessage(content)
        self.messageManager.sendMessage(SIMChatBaseMessageTextContent(content: locationNameShared))
    }
